...
- 페어는 두 명 씩
- 홀수 (3,5,6 인 경우) 한 페어는 3인
- 이전 매칭 시 다음 매칭 불가
- List에서 앞 두명씩 페어
- 홀 수 인 경우 마지막 크루는 마지막 페어에 포함
- 매칭 전적이 있는 경우 크루 목록 다시 래덤
- 최대 3회 시도 or 매칭 불가 ( 전적) 에러 메시지 출력
- 안내 문구 출력 후 매칭 진행
- 아니오 할 경우 코스, 레벨, 미션 선택
- 페어 조회 기능
  - 과정,레벨,미션 → 페어 정보 출력
  - 매칭 이력없으면 없다고 안내 → 에러
...
- 페어를 어떻게 저장할 것인가?
- 페어 했던 여부를 어떻게 조회할 것 인가?
  - List<String> 에서 뽑았을 때, 두 개의 이름이 반환
  - 두 이름을 가지고 얘네가 서로 페어였었는지(같은 과정, 같은 레벨 이었는지) 어떻게 확인?
    1. 크루별 객체 생성
      1. 각 크루별로 이름, 과정, 레벨, 레벨 별 페어 목록을 저장
      2. 페어 여부 조회 시,
        1. 크루 객체 조회
          1. 페어 목록 조회
            1. 페어 목록에서 상대 이름 있는지 조회
    2. 페어 정보 자체를 따로 저장
      1. 각 과정, 미션에 따른 페어 정보를 저장 List<Pair<>>와 같이
      2. 페어 여부 조회 시,
        1. 페어 정보 조회 ( 비교해야할 페어 정보가 미션마다 많아짐)
          1.
          2. 페어 정보에서 해당 이름 두 개를 가지는 값이 있는 지 조회
             → 이러면 이전 전력을 조회할 수 없음 예를 들어 1미션에 만들때, 2미션, 3미션 전력은 조회가 불가능
...
- List<String>을 셔플해서 앞서 두 개를 추출
  - 크루 객체를 조회
  - 크루 객체에서 hasMet 값에서 서로 이름을 검색
  - 없으면 페어
  - 있으면 재 추출
- List<List<>>로 각 이름 저장해서 반환

반환

- List<List<>>
  - pair로 할 경우 3명의 경우를 지원 못함
  - 유동적인 List 사용
